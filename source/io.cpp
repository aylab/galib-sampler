...
/* parse_ranges_file reads the given buffer and stores every range found in the given ranges array
	...
	todo:
*/
void parse_ranges_file (char* buffer, input_params& ip) {
	int i = 0;
	int rate = 0;
	for (; buffer[i] != '\0'; i++) {
		...
		// Read the bounds
		int lower_bound = atof(buffer + i);
		while (buffer[i] != ',') {i++;}
		i++;
		int upper_bound = atof(buffer + i);
		if (lower_bound < 0 || upper_bound < 0) { // If the ranges are invalid then set them to 0
			lower_bound = 0;
			upper_bound = 0;
		}
		ip.ranges[rate].first = lower_bound;
		ip.ranges[rate].second = upper_bound;
...
/* open_file opens the file with the given name and stores it in the given output file stream
	...
	todo:
*/
void open_file (ofstream* file_pointer, const char* file_name, bool append) {
	ostream& v = term->verbose();
...
double simulate_set (input_params& ip, double parameters[]) {
	...
	if (pid == 0) {
		...
		child_pid = pid;
		term->verbose() << term->blue << "Done: " << term->reset << "the child process's PID is " << child_pid << endl;
	}
	const char* grad_fname = "input.gradients";
