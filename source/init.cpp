...
void accept_input_params (int num_args, char** args, input_params& ip) {
	...
	if (num_args > 1) { // If arguments were given (the 0th argument is the program name)
		for (int i = 1; i < num_args; i += 2) { // Iterate through each argument pair (if an argument does not have an accompanying value, i-- should be called when found)
			...
			} else if (option_set(option, "-p", "--population")) {
				ensure_nonempty(option, value);
				ip.population = atoi(value);
				if (ip.population < 1) {
					usage("The population must be at least one simulation. Set -p or --population to at least 1.");
				}
			...
			} else if (option_set(option, "-m", "--mutation-prob")) {
				ensure_nonempty(option, value);
				ip.prob_mutation = atof(value);
				if (ip.prob_mutation < 0 || ip.prob_mutation > 1) {
					usage("The probability of a mutation must be a valid probability. Set -m or --mutation-prob to between 0 and 1, inclusive.");
				}
			} else if (option_set(option, "-C", "--crossover-prob")) {
				ensure_nonempty(option, value);
				ip.prob_crossover = atof(value);
				if (ip.prob_crossover < 0 || ip.prob_crossover > 1) {
					usage("The probability of a crossover must be a valid probability. Set -C or --crossover-prob to between 0 and 1, inclusive.");
				}
...
/* read_ranges fills in the sres_params struct with ranges from the given ranges file
	...
	todo:
*/
void read_ranges (input_params& ip, input_data& ranges_data) {
	read_file(&ranges_data);
	ip.ranges = new pair<int, int>[ip.num_dims];
	parse_ranges_file(ranges_data.buffer, ip);
}
